...
| 6 | 分析圖：摺線（支出／收入／現金淨資產）＋分類圓餅 | `iH20573wrb` | **設計好**（2026-09-07 驗收※） | 已有 | **無獨立頁**（圖還在首頁） | build 須新建／搬移 | ※摺線目前僅「支出」；收入／現金淨資產切換＝設計 follow-up，不擋本批 |
...
**Follow-up（不擋本批設計驗收，請毛利蘭補一幀或改分析頁）：**
- 分析圖摺線需可切 **收入**、**現金淨資產（不含持倉）**（規格：預設支出，可切兩系列）。現幀 `iH20573wrb` 僅「支出趨勢」。
